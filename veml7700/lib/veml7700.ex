defmodule VEML7700 do
  @moduledoc """
  Module to interface with the VEML7700 ambient light sensor.
  """
  use GenServer
  ...
  def init(%{address: address, i2c_bus_name: bus_name} = args) do
    ...
    config = args |> Map.take([:gain, :int_time, :shutdown, :interrupt]) |> Config.new()
  ...
  def init(args) do
    ...
    Logger.info("Starting VEML7700. Please specify and address and a bus.")
    Logger.info("Starting on " <> transport)

    defaults = args |> Map.put(:address, address) |> Map.put(:i2c_bus_name, bus_name)
